    ...
    <main id="main" className="min-h-dvh bg-[--bg]">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-6 animate-fade-up">
        ...
        <div className="grid grid-cols-3 gap-3">
          <Link
            href="/for-you"
            className="flex items-center gap-3 bg-cream-raised rounded-xl p-4 border border-[#e8d9bf] hover:border-terra/40 transition-all group"
          >
            <div className="w-8 h-8 rounded-lg bg-terra/10 flex items-center justify-center">
              <Sparkles className="w-4 h-4 text-terra" />
            </div>
            <div className="min-w-0">
              <p className="text-sm font-bold text-ink">For You</p>
              <p className="text-[10px] text-ink-light">Your matched feed</p>
            </div>
          </Link>
          <Link
            href="/saved"
            className="flex items-center gap-3 bg-white rounded-xl p-4 border border-sand-light hover:border-terra/30 transition-all group"
          >
            <div className="w-8 h-8 rounded-lg bg-sage/10 flex items-center justify-center">
              <Star className="w-4 h-4 text-sage" />
            </div>
            <div className="min-w-0">
              <p className="text-sm font-bold text-ink">{savedCount + goingCount}</p>
              <p className="text-[10px] text-ink-light">Saved</p>
            </div>
          </Link>
          <Link
            href="/leaderboard"
            className="flex items-center gap-3 bg-white rounded-xl p-4 border border-sand-light hover:border-terra/30 transition-all group"
          >
            <div className="w-8 h-8 rounded-lg bg-turq/10 flex items-center justify-center">
              <Trophy className="w-4 h-4 text-turq" />
            </div>
            <div className="min-w-0">
              <p className="text-sm font-bold text-ink">
                {myRank >= 0 ? `#${myRank + 1}` : 'Unranked'}
              </p>
              <p className="text-[10px] text-ink-light">Leaderboard</p>
            </div>
          </Link>
        </div>
          ...
          <section>
            ...
            <div className="space-y-2">
              {checkIns!.slice(0, 5).map(ci => (
                <div key={ci.id} className="flex items-center gap-3 bg-white rounded-xl px-4 py-3 border border-sand-light">
                  <CheckSquare className="w-4 h-4 text-sage flex-shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-ink truncate">{ci.event_name}</p>
                    <p className="text-[10px] text-ink-light">{ci.venue_name}</p>
                  </div>
                  <p className="text-[10px] text-ink-light flex-shrink-0">
        ...
        {totalCheckins === 0 && (
          <div className="text-center py-8 bg-white rounded-2xl border border-sand-light">
            <Zap className="w-10 h-10 text-terra mx-auto mb-3" />
            <h3 className="text-base font-bold text-ink mb-1" style={{ fontFamily: 'var(--font-epilogue)' }}>
              Time to get out there
            </h3>
            <p className="text-xs text-ink-light mb-4 max-w-xs mx-auto">
        ...
        <section>
          ...
          <div className="bg-white rounded-xl border border-sand-light divide-y divide-sand-light">
            <div className="p-4 flex items-center justify-between gap-3">
              <div className="flex items-start gap-3">
                <Bell className="w-4 h-4 text-terra mt-0.5 flex-shrink-0" />
                <div>
                  <p className="text-sm font-semibold text-ink">Event reminders</p>
                  <p className="text-[10px] text-ink-light mt-0.5">Push notifications for events you&apos;re going to</p>
                </div>
              </div>
              <PushBell />
            </div>
            <Link
              ...
              className="p-4 flex items-center justify-between gap-3 hover:bg-cream/60 transition-colors"
            >
              <div className="flex items-start gap-3">
                <Bell className="w-4 h-4 text-sage mt-0.5 flex-shrink-0" />
                <div>
                  <p className="text-sm font-semibold text-ink">What to hear about</p>
                  <p className="text-[10px] text-ink-light mt-0.5">Categories, venues, artists, neighborhoods. Only get notified about stuff you care about.</p>
                </div>
              </div>
        ...
        {/* How to Use guide */}
        <section>
          <h2 className="text-base font-black text-ink mb-3 flex items-center gap-2" style={{ fontFamily: 'var(--font-epilogue)' }}>
          ...
          <div className="bg-white rounded-xl border border-sand-light divide-y divide-sand-light">
            {[
              {
                icon: <Heart className="w-4 h-4 text-terra" />,
              ...
              {
                icon: <Search className="w-4 h-4 text-turq" />,
                title: 'Find friends',
              ...
              {
                icon: <UserPlus className="w-4 h-4 text-sage" />,
                title: 'Follow people',
              ...
              {
                icon: <Users className="w-4 h-4 text-terra" />,
              ...
              {
                icon: <Trophy className="w-4 h-4 text-turq" />,
                title: 'Earn badges',
              ...
              {
                icon: <Zap className="w-4 h-4 text-terra" />,
                title: 'Surprise Me',
              ...
              <div key={title} className="flex items-start gap-3 px-4 py-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-ink-light mt-0.5 leading-relaxed">{desc}</p>
                </div>
              </div>
            ))}
          </div>
