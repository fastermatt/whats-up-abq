...
export default function SubmitEventPage() {
  ...
  const inputClass = (hasError?: boolean) =>
    `w-full px-3 py-2.5 rounded-xl border text-sm text-ink bg-white placeholder-ink-light focus-visible:outline-none focus-visible:ring-2 transition ${
      hasError
        ? 'border-red-400 focus:border-red-400 focus-visible:ring-red-200'
        : 'border-sand-mid focus:border-terra focus-visible:ring-terra/20'
    }`
  ...
  const Req = () => <span className="text-terra" aria-label="required">*</span>

  if (!authChecked) {
    return (
      <main id="main" className="min-h-dvh bg-cream flex items-center justify-center">
      ...
      <main id="main" className="min-h-dvh bg-cream flex items-center justify-center px-4">
        <div className="max-w-md w-full text-center animate-fade-up">
          ...
          <div className="flex flex-col items-center gap-3">
            <Link
              href="/events"
              className="inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-full bg-terra text-white text-base font-bold hover:bg-terra-hover transition-colors min-w-[220px]"
            ...
            <button
              ...
              className="text-xs text-ink-light hover:text-terra transition-colors underline underline-offset-2"
            >
  ...
  return (
    <main id="main" className="min-h-dvh bg-cream">
      ...
      <div className="bg-cream border-b border-sand-light">
        <div className="max-w-2xl mx-auto px-4 pt-7 pb-6">
          <p className="text-[10px] uppercase tracking-[0.2em] text-terra font-semibold mb-2">
            Community submissions
          </p>
          <h1 className="text-2xl sm:text-3xl font-black leading-tight mb-2 text-ink" style={{ fontFamily: 'var(--font-epilogue)' }}>
            Submit your event
          </h1>
          <p className="text-sm text-ink-mid leading-relaxed max-w-md">
          ...
          <div className="mt-4 flex items-center gap-2 bg-white border border-sand-light rounded-xl px-3.5 py-2.5 w-fit">
            <Info className="w-3.5 h-3.5 text-terra flex-shrink-0" aria-hidden="true" />
            <p className="text-[11px] text-ink-mid leading-relaxed">
              Approved events show a <span className="font-semibold text-ink">Community</span> badge and credit you as the submitter.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        <p className="text-[11px] text-ink-light mb-5">
          All fields marked <span className="text-terra font-bold">*</span> are required.
        </p>

        <form onSubmit={handleSubmit} noValidate className="space-y-4">

          {/* ── Event core ── */}
          <section aria-labelledby="submit-section-event" className="bg-white rounded-2xl border border-sand-light p-5 space-y-4">
            <h2 id="submit-section-event" className="text-xs font-bold text-terra uppercase tracking-wider"><span className="text-terra/55">Step 1 of 4 ·</span> The event</h2>

            <div>
              <label htmlFor="title" className="block text-xs font-semibold text-ink-mid mb-1.5">
                Event name <Req />
              </label>
              <input
                id="title"
            ...
            <div>
              <label htmlFor="description" className="block text-xs font-semibold text-ink-mid mb-1.5">
                Description <Req />
              </label>
              <textarea
                id="description"
              ...
              <div className="flex items-center justify-between mt-1">
                ...
                <p id="description-counter" className="text-[10px] text-ink-light ml-auto">{form.description.length}/2000</p>
              </div>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                <label htmlFor="event_date" className="block text-xs font-semibold text-ink-mid mb-1.5 flex items-center gap-1">
              ...
              <div>
                <label htmlFor="start_time" className="block text-xs font-semibold text-ink-mid mb-1.5">
                  Start time <Req />
                </label>
                <input
                  id="start_time"
              ...
              <div>
                <label htmlFor="end_time" className="block text-xs font-semibold text-ink-mid mb-1.5">End time</label>
                <input
                  id="end_time"
            ...
            <div>
              <label htmlFor="category" className="block text-xs font-semibold text-ink-mid mb-1.5 flex items-center gap-1">
          ...
          {/* ── Venue ── */}
          <section aria-labelledby="submit-section-venue" className="bg-white rounded-2xl border border-sand-light p-5 space-y-4">
            <h2 id="submit-section-venue" className="text-xs font-bold text-terra uppercase tracking-wider"><span className="text-terra/55">Step 2 of 4 ·</span> Venue</h2>
            <div>
              <label htmlFor="venue_name" className="block text-xs font-semibold text-ink-mid mb-1.5 flex items-center gap-1">
            ...
            <div>
              <label htmlFor="venue_address" className="block text-xs font-semibold text-ink-mid mb-1.5">
                Address <Req />
              </label>
              <input
                id="venue_address"
          ...
          <section aria-labelledby="submit-section-photo" className={`rounded-2xl p-5 space-y-4 border-2 transition-colors ${
            fieldErrors.photo
              ...
              : photoFile
                ? 'bg-[#f0f9f5] border-sage/40'
                : 'bg-gradient-to-br from-[#fff8f4] to-[#fdf3ec] border-terra/30'
          }`}>
            <div className="flex items-start justify-between">
              <div>
                <p className="text-[10px] font-bold text-terra/55 uppercase tracking-wider mb-1">Step 3 of 4</p>
                <div className="flex items-center gap-2 mb-0.5">
                  <ImageIcon className="w-4 h-4 text-terra" />
                  <h2 id="submit-section-photo" className="text-sm font-black text-ink" style={{ fontFamily: 'var(--font-epilogue)' }}>
                    Event Photo <Req />
                  </h2>
                </div>
              ...
              {!photoFile && (
                <span className="flex-shrink-0 bg-terra text-white text-[9px] font-black uppercase tracking-wider px-2 py-1 rounded-full">
                  Required
                </span>
              )}
              {photoFile && (
                <span className="flex-shrink-0 bg-sage text-white text-[9px] font-black uppercase tracking-wider px-2 py-1 rounded-full flex items-center gap-1">
              ...
              <div className="grid grid-cols-2 gap-1.5">
                {[
                  ...
                  <div key={label as string} className="flex items-center gap-1.5 bg-white/70 rounded-lg px-2.5 py-1.5">
                    {(() => { const C = Icon as React.ComponentType<{ className?: string }>; return <C className="w-3.5 h-3.5 text-terra flex-shrink-0" /> })()}
                    <span className="text-[10px] text-ink-mid font-medium leading-snug">{label as string}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            {photoPreview ? (
              <div className="relative">
                ...
                <div className="absolute inset-0 rounded-xl ring-2 ring-sage/30 pointer-events-none" />
              ...
              <label htmlFor="photo" className={`flex flex-col items-center justify-center w-full aspect-[16/10] border-2 border-dashed rounded-xl cursor-pointer transition-all hover:scale-[1.01] ${
                fieldErrors.photo ? 'border-red-300 bg-red-50/50' : 'border-terra/30 bg-white/60 hover:bg-white/90 hover:border-terra/60'
              }`}>
                <div className="flex flex-col items-center gap-2 text-center p-4">
                  <div className="w-12 h-12 rounded-full bg-terra/10 flex items-center justify-center">
                    <Upload className="w-6 h-6 text-terra" />
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-ink-mid">Tap to upload photo</p>
                    <p className="text-[10px] text-ink-light mt-0.5">Landscape · min 800×500 · JPG/PNG/WebP · max 5 MB</p>
                  </div>
                </div>
                <input
                  id="photo"
            ...
            {status === 'submitting' && photoFile && uploadProgress > 0 && uploadProgress < 100 && (
              <div className="space-y-1">
                <p className="text-[10px] text-ink-light">Uploading photo… {uploadProgress}%</p>
                <div className="h-1.5 bg-sand-light rounded-full overflow-hidden">
                  <div className="h-full bg-terra transition-all duration-300" style={{ width: `${uploadProgress}%` }} />
                </div>
              </div>
            )}
          </section>
          ...
          {/* ── Tickets & price ── */}
          <section aria-labelledby="submit-section-tickets" className="bg-white rounded-2xl border border-sand-light p-5 space-y-4">
            <h2 id="submit-section-tickets" className="text-xs font-bold text-terra uppercase tracking-wider"><span className="text-terra/55">Step 4 of 4 ·</span> Tickets &amp; price <Req /></h2>

            <label htmlFor="is_free" className="flex items-center gap-2.5 text-sm text-ink-mid cursor-pointer select-none">
              <input
                id="is_free"
            ...
            {!form.is_free && (
              <>
                <div>
                  <label htmlFor="ticket_url" className="block text-xs font-semibold text-ink-mid mb-1.5">
                    Ticket URL <Req />
                  </label>
                  <input
                    id="ticket_url"
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label htmlFor="price_min" className="block text-xs font-semibold text-ink-mid mb-1.5 flex items-center gap-1">
                  ...
                  <div>
                    <label htmlFor="price_max" className="block text-xs font-semibold text-ink-mid mb-1.5">Max price</label>
                    <input
                      id="price_max"
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-6 py-4 rounded-xl bg-terra text-white font-bold text-sm hover:bg-terra-hover active:scale-[0.98] transition-all disabled:opacity-60 disabled:cursor-not-allowed shadow-md shadow-terra/20"
            style={{ fontFamily: 'var(--font-epilogue)' }}
          >
            {status === 'submitting' ? (
              <>
          ...
          <p className="text-[10px] text-ink-light text-center pb-6">
