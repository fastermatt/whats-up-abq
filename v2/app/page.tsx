...
// Streaming SSR: the hero h2 renders before data resolves — it is not blocked
// by Supabase/Redis queries. Redis cache serves data in ~50-100ms after warm-up.
// revalidate=60 hints to Netlify's CDN to cache fully-rendered pages when ISR
// is available, so repeat visitors see instant loads.
// force-static removed: it prevented Suspense streaming (pre-renders everything
// at build time, blocking h2 on all data even for cold ISR requests).
...
/** Redis-cache wrapper with Supabase fallback. */
async function rc<T>(key: string, fn: () => Promise<T>, ttl = 300): Promise<T> {
  try { return await cachedFetch(key, fn, ttl) }
  catch { return fn() }
}

// Per-request cached fetchers — React cache() deduplicates calls within one
// server render, so HeroDynamic + HeroStatStrip + EventDataSections all share
// the same in-flight promises when they request the same data.
const getTonight    = cache(() => rc('hp:tonight',  () => fetchEvents({ timeFilter: 'tonight',      limit: 10 }), 300))
const getWeekend    = cache(() => rc('hp:weekend',  () => fetchEvents({ timeFilter: 'this-weekend', limit: 10 }), 900))
const getUpcoming   = cache(() => rc('hp:upcoming', () => fetchEvents({ timeFilter: 'upcoming',     limit: 1  }), 600))
const getFeatured   = cache(() => rc('hp:featured', () => fetchFeaturedEvents(6),                                 900))
const getNeighhoods = cache(() => rc('hp:hoods',    () => fetchNeighborhoodCounts(),                              3600))
const getMovies     = cache(() => rc('hp:movies',   () => fetchNowPlayingMovies(10),                              3600))

// ── Main page — non-async so the hero h2 ships with the first HTML chunk ──────
...
export default function DiscoverPage() {
  ...
  return (
    <main id="main" className="min-h-dvh bg-[--bg]">
      ...
      {/* ── Hero — h2 renders immediately, dynamic counts stream in ── */}
      <HeroShell />
      ...
      {/* ── Explore ABQ — places static, neighborhoods stream ── */}
      <ExploreShell featuredPlaces={featuredPlaces} />
...
// ── Hero shell — static frame renders immediately with the first HTML chunk ───
    ...
    <section className="relative overflow-hidden" style={{ background: '#eedcd0' }}>
      ...
      >
        <div className="absolute animate-map-pan" style={{ top: '-15%', bottom: '-15%', left: '-6%', right: '-6%' }}>
          {/* Map removed — heading text is now the LCP element */}
        </div>
      </div>
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 pt-5 md:pt-7 pb-5">
        ...
        {/* ── LCP ELEMENT — no data dependency, renders with first HTML chunk ── */}
        <h2
        ...
        {/* Dynamic: heroSaying + tonight pill + search with event count.
            Fallback renders a functional search bar immediately. */}
        <Suspense fallback={<HeroContentFallback />}>
          <HeroDynamic />
        </Suspense>
      </div>

      {/* Stat strip — streams after data loads */}
...
// Fallback: functional search bar without counts — renders with first HTML chunk
function HeroContentFallback() {
  return (
    <>
      {/* Spacer matching heroSaying line height so stat strip doesn't jump */}
      <div className="h-[1.4rem] mb-5" aria-hidden="true" />
...
// Fallback: stat strip with dash placeholders
  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto grid grid-cols-3">
        {(['Tonight', 'This Weekend', 'All Upcoming'] as const).map((label, i) => (
          <div
            key={label}
            className="py-3.5 flex flex-col items-center"
            style={i < 2 ? { borderRight: '1px solid rgba(26,22,20,.08)' } : {}}
          >
            <span
              className="font-black text-xl sm:text-2xl leading-none text-[#1a1614]"
              style={{ fontFamily: 'var(--font-epilogue)' }}
            >
              —
            </span>
...
// Async: heroSaying + tonight pill + search bar with event count
async function HeroDynamic() {
  const [tonight, allUpcoming] = await Promise.all([getTonight(), getUpcoming()])

  const abqHour = parseInt(
    new Date().toLocaleString('en-US', { hour: 'numeric', hour12: false, timeZone: 'America/Denver' })
  )
...
// Async: stat strip with live counts
async function HeroStatStrip() {
  const [tonight, weekend, allUpcoming] = await Promise.all([
    getTonight(), getWeekend(), getUpcoming(),
  ])
...
// Async: featured, tonight, and weekend sections — all stream together
async function EventDataSections() {
  const [tonight, weekend, featured] = await Promise.all([
    getTonight(), getWeekend(), getFeatured(),
  ])
...
// ── Explore ABQ — places (static) + neighborhoods (async) ────────────────────

function ExploreShell({ featuredPlaces }: { featuredPlaces: Place[] }) {
  return (
    <AnimateIn animation="fade-up" delay={130}>
      <section className="py-10 bg-gradient-to-b from-[#f5ece3] to-[#fbf7f1] border-y border-[#e8d5c0]/70">
        ...
        {/* Places row — static, renders immediately */}
        <div className="mb-8">
        ...
        {/* Neighborhoods — async, streams after Redis resolves */}
        <Suspense fallback={null}>
          <NeighborhoodGrid />
        </Suspense>
      </section>
...
async function NeighborhoodGrid() {
  const neighborhoodCounts = await getNeighhoods()
  if (!neighborhoodCounts.length) return null
